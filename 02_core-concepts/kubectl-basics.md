...
### `run` vs `create` vs `apply` — 헷갈리지 말기

셋 다 "리소스를 만든다"지만 **방식**과 **만드는 대상**이 다르다.

| 명령 | 방식 | 만드는 것 | 이미 있으면? | 주 용도 |
|---|---|---|---|---|
| `kubectl run` | 명령형 | **Pod 하나** | 에러(AlreadyExists) | 단발 Pod·디버깅 (`--rm -it`) |
| `kubectl create` | 명령형 | 여러 종류(deploy/svc/cm/secret/ns/job…) | **에러** | 리소스 빠르게 **새로** 생성 |
| `kubectl apply` | **선언형** | 파일(`-f`)이 정의한 무엇이든 | **갱신(patch)** | 매니페스트로 생성+수정, **멱등** |

- **run vs create**: `run`은 **Pod 전용 단축**(예전엔 Deployment/Job도 만들었지만 지금은 Pod만), `create`는 **여러 리소스 종류**를 만든다. 둘 다 명령형이라 같은 이름이 있으면 에러.
- **create vs apply**(진짜 중요한 차이): 같은 파일을 **다시 실행**하면 —
  ```bash
  kubectl create -f web.yaml     # 두 번째 → "AlreadyExists" 에러
  kubectl apply  -f web.yaml     # 몇 번이든 OK: 없으면 생성, 있으면 바뀐 필드만 갱신(멱등)
  ```
  그래서 **파일을 Git에 두고 운영하는 선언형 워크플로의 기본은 `apply`**.

> `apply`는 적용 내용을 `last-applied-configuration` 어노테이션에 기록해, 다음 `apply` 때 **3-way 비교**로 바뀐 부분만 패치한다. (`create -f`로 만든 걸 나중에 `apply -f`하면 어노테이션이 없다는 경고가 한 번 뜨고 넘어간다.)
>
> 💡 **시험 전략**: 간단한 건 `run`/`create`로 빠르게, **복잡·수정 필요한 건 아래 `--dry-run`으로 YAML 뽑아 `apply`**.
...
### `--dry-run` 값 세 가지 (`client`만 있는 게 아니다)

`--dry-run`은 `none`(기본) / `client` / `server` 중 하나다. (예전 `--dry-run=true/false`는 1.18부터 이걸로 대체.)

| 값 | 처리 위치 | etcd 저장 | 용도 |
|---|---|---|---|
| `none` (기본) | 서버 | ✅ | `--dry-run` 안 쓴 평소 — **실제 적용** |
| `client` | **kubectl만** | ❌ | 보낼 객체를 로컬에서 출력. 빠름·클러스터 무관. **YAML 뽑기용** |
| `server` | **서버**(검증·기본값·admission·webhook) | ❌ | 서버가 처리하되 저장만 안 함. **실제 통과 여부·기본값 채워진 결과 확인** |

```bash
kubectl create deploy web --image=nginx:1.27 --dry-run=client -o yaml   # 내가 쓴 필드만, 깔끔
kubectl create deploy web --image=nginx:1.27 --dry-run=server -o yaml   # 서버 기본값까지 채워진 전체 모습
```
- **`client`**: 클러스터에 안 물어봐서 빠르고, admission 거부·CRD 스키마 위반 같은 **서버측 문제는 못 잡는다.** 출력이 깔끔해 **매니페스트 뼈대 생성에 최적**(그래서 시험은 거의 `client`).
- **`server`**: "실제로 적용되면 어떻게 될까"를 본다 — 충돌·webhook 거부까지 확인 가능. `kubectl apply --dry-run=server`로 **적용 시 변경분 미리보기**, `kubectl diff -f f.yaml`도 내부적으로 server dry-run을 쓴다.
