...
| `11_identity-keycloak` | Keycloak(realm·client·토큰), AD 연동(LDAP) | 실무 (CKA 외) |
| `12_data-stores` | PostgreSQL(OLTP)·ClickHouse(OLAP), DBeaver·Grafana·Langfuse | 실무 (CKA 외) |
...
- 주제별 **실습 가이드는 `practice.md`** 로 둔다(Claude가 작성). 단계별 명령 + 🔎관찰 포인트 + 🧪과제로 구성 — 사용자가 따라 실행한다. **내용이 길어지면 `practice1.md`·`practice2.md`…로 나누고** 각 README가 모두 가리키게 한다(예: `02_core-concepts`). **`practice.md`는 OS 중립으로 쓴다**(`kubectl` 위주). colima 등 OS별 환경 시작 명령은 넣지 말고 `01_lab-environment`를 참고하게 한다(사용자는 macOS/Ubuntu 둘 다 가능).
- **OS별 설치/도구 안내는 macOS·Ubuntu를 나란히 준다.** 사용자는 두 환경을 모두 쓴다. OS에 따라 갈리는 설치 명령(예: GUI 클라이언트, CLI 도구)이 필요하면 macOS(`brew …`)와 Ubuntu(`apt`/`snap` 또는 공식 배포 페이지)를 함께 적는다. `practice.md` 본문 자체는 위 규칙대로 OS 중립을 유지한다.
...
- 상단 주석에 **핵심이 되는 `kind`의 공식 문서 링크**를 넣는다(예: `# 📖 Pod: https://kubernetes.io/docs/concepts/workloads/pods/`). 매니페스트만 봐도 해당 리소스 문서로 바로 갈 수 있게.
...
- **인증/SSO(Keycloak) → `11`**, **데이터 스토어(PostgreSQL·ClickHouse + DBeaver·Grafana·Langfuse) → `12`** (둘 다 CKA 외 실무). DB 매니페스트는 12에 두되, Postgres는 ArgoCD 데모용으로 `10`에 이미 있는 것을 재사용.
