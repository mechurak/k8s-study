...
- 주제별 **실습 가이드는 `practice.md`** 로 둔다(Claude가 작성). 단계별 명령 + 🔎관찰 포인트 + 🧪과제로 구성 — 사용자가 따라 실행한다. **`practice.md`는 OS 중립으로 쓴다**(`kubectl` 위주). colima 등 OS별 환경 시작 명령은 넣지 말고 `01_lab-environment`를 참고하게 한다(사용자는 macOS/Ubuntu 둘 다 가능).
