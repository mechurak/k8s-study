...
실습 환경 구조는 **OS에 따라 층 수가 다르다.** 위층이 꺼지면 아래층도 멈추므로 **시작은 위→아래, 정리는 아래→위** 순서로 한다.

```
macOS : colima (리눅스 VM)  ──▶  docker (런타임)  ──▶  kind 클러스터 (컨테이너 = 노드)
Ubuntu:                          docker (네이티브)  ──▶  kind 클러스터 (컨테이너 = 노드)
```

> **핵심: 어느 OS든 일상 종료에 `kind delete`를 쓰지 않는다.** kind 클러스터(노드 컨테이너)는 그대로 두고 **런타임만 껐다 켜면** 복구된다. `kind delete`/`kind create`는 "클러스터를 통째로 버리고 새로 만들" 때만 — 매일 하는 게 아니다.
> - **macOS**: 클러스터가 colima VM 디스크에 저장 → `colima stop`/`start`로 멈췄다 재개(노드 컨테이너 자동 복구).
> - **Ubuntu**: VM이 없으니 **노드 컨테이너 자체를** `docker stop`/`start` 하거나 docker 데몬을 멈췄다 켠다(아래 Ubuntu 루틴).
...
### 시작 / 종료 (Ubuntu, 네이티브 docker)

VM이 없으니 **kind 노드 컨테이너(또는 docker 데몬)를** 껐다 켜는 게 macOS의 `colima stop`/`start`에 해당한다. **클러스터는 보존**된다 — `kind delete`가 아니다.

```bash
# 끝낼 때 — 노드 컨테이너 정지 (호스트 자원 회수, 클러스터 보존)
docker stop $(kind get nodes --name study)
#   또는 docker 엔진째: sudo systemctl stop docker

# 시작할 때 — 다시 켜기
docker start $(kind get nodes --name study)
kubectl get nodes      # control plane 복구에 수십 초 걸릴 수 있음
```
- `kind get nodes --name study`가 노드 컨테이너 이름(`study-control-plane`, `study-worker`…)을 출력 → 한 번에 stop/start.
- 호스트 재부팅·docker 데몬 재시작 뒤 노드가 멈춰 있으면 위 `docker start ...`로 깨운다(자동 재시작 보장 안 됨).
- **완전 폐기**는 macOS와 동일: `kind delete cluster --name study`. Ubuntu엔 지울 VM이 없으니, 더 비우려면 `docker system prune` 정도만 추가.
...
| 환경 층 구조 | 3층 (colima VM → docker → kind) | 2층 (docker → kind, VM 없음) |
| 일상 시작/종료 | `colima start` / `colima stop` (클러스터 보존) | `docker start/stop $(kind get nodes --name study)` 또는 데몬 on/off (클러스터 보존) |
