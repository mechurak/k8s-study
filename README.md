...
3. **순서** — 폴더 번호 순(01 → 12) 또는 아래 [학습 로드맵](#️-학습-로드맵)을 참고.
...
| [12_data-stores](./12_data-stores/) | PostgreSQL(OLTP)·ClickHouse(OLAP), DBeaver·Grafana·Langfuse | 실무 (CKA 외) |

> Helm·Kustomize는 06번(공식 커리큘럼상 Cluster Architecture)에, ArgoCD 등 GitOps 도구는 10번, 인증/SSO(Keycloak)는 11번, 데이터 스토어(PostgreSQL·ClickHouse)는 12번에 정리한다.
...
5. **병행 / 이후 — 실무** (09~12번): EKS 직접 배포, ArgoCD/GitOps, Keycloak 인증/SSO·AD 연동, PostgreSQL·ClickHouse 데이터 스토어(+Langfuse)
